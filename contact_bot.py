...
async def handle_contact_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == bot_owner_id:
        ...
        # 6. الخطوة 4: استلام السعر والطلب يوزر المدرب
            ...
            await update.message.reply_text("👨‍🏫 **الخطوة 5:** أرسل (يوزرنايم) المدرب مع الـ @ أو أرسل **المعرف الرقمي (ID)** مباشرة:")
            return

        # 7. الخطوة 5: استلام المدرب (تم التصحيح ليدعم الرقم واليوزر)
        elif action == 'awaiting_crs_coach':
            input_val = text.strip()
            from sheets import find_user_by_username
            
            # فحص: هل المدخل رقم (ID)؟
            if input_val.isdigit():
                context.user_data['temp_crs'].update({
                    'coach_user': "إدخال يدوي",
                    'coach_id': input_val,
                    'coach_name': f"مدرب (ID: {input_val})"
                })
                # محاولة تحسين الاسم إذا كان المدرب قد راسل البوت
                try:
                    c_chat = await context.bot.get_chat(input_val)
                    context.user_data['temp_crs']['coach_name'] = c_chat.full_name
                except: pass

                context.user_data['action'] = 'awaiting_crs_date'
                await update.message.reply_text(f"✅ تم قبول المعرف الرقمي بنجاح.\n\n🗓 **الخطوة 6:** أرسل تاريخ بداية الدورة:")
            
            # فحص: هل المدخل يوزرنايم؟
            else:
                ...
                if user_data:
                    ...
                    try:
                        ...
                        await update.message.reply_text(
                            "❌ **فشل العثور على المدرب عبر اليوزر!**\n\n"
                            "💡 يرجى إرسال **المعرف الرقمي (ID)** للمدرب الآن بدلاً من اليوزر."
                        )
            return
