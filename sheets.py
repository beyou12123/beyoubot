...
#دالة جلب عدد البوتات الكلي
def get_total_bots_count():
    """حساب عدد كافة البوتات المصنوعة في المصنع حالياً"""
    global bots_sheet
    if bots_sheet is None: connect_to_google()
    try:
        # جلب عدد الصفوف التي تحتوي على بيانات (باستثناء العنوان)
        return len(bots_sheet.col_values(1)) - 1
    except:
        return 0

# --------------------------------------------------------------------------
def get_bot_users_count(bot_token):
    """جلب عدد مستخدمي بوت معين (تجريبي - يمكنك ربطها بورقة الإحصائيات لاحقاً)"""
    # حالياً سنعيد قيمة عشوائية أو ثابتة حتى تربطها بجدول المستخدمين الخاص بكل بوت
    return 1 # يمكنك تطويرها لاحقاً لتقرأ من شيت الإحصائيات

def get_bot_blocks_count(bot_token):
    """جلب عدد المحظورين لبوت معين"""
    return 0 # يمكنك تطويرها لتقرأ من شيت السجلات

# --------------------------------------------------------------------------
def get_total_factory_users():
    """حساب عدد مستخدمي المصنع الكلي من شيت المستخدمين"""
    global users_sheet
    if users_sheet is None: connect_to_google()
    try:
        # جلب عدد الصفوف التي تحتوي على بيانات في عمود الآيدي
