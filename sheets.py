...
def save_user(user_id, username, inviter_id=None):
    """
    تسجيل مستخدم جديد مع ضمان عدم التكرار ودعم نظام الإحالة
    العمود 10: معرف إحالة
    العمود 11: رصيد
    """
    global users_sheet
    ...
    try:
        # 1. التحقق من وجود المستخدم مسبقاً (منع التكرار)
        # البحث في العمود الأول (ID المستخدم)
        try:
            exists = users_sheet.find(str(user_id), in_column=1)
            if exists: 
                return False # المستخدم مسجل مسبقاً، لا نفعل شيئاً
        except: 
            pass # لم يجد المستخدم، وهذا هو المطلوب للتسجيل الجديد

        now = get_system_time("full")
        
        # 2. معالجة معرف الإحالة (العمود 10)
        # إذا دخل المستخدم عبر رابط (ref_ID)، نضع ID المحيل، وإلا نتركه فارغاً
        ref_id = str(inviter_id) if inviter_id else ""
        
        # 3. إعداد صف البيانات (11 عمود حسب الترتيب المطلوب)
        # [0]ID المستخدم, [1]اسم المستخدم, [2]تاريخ التسجيل, [3]الحالة, [4]نوع الاشتراك, 
        # [5]عدد البوتات, [6]آخر نشاط, [7]اللغة, [8]مصدر التسجيل, [9]معرف إحالة, [10]رصيد
        row = [
            str(user_id),       # ID المستخدم
            str(username),      # اسم المستخدم
            now,                # تاريخ التسجيل
            "نشط",              # الحالة
            "مجاني",            # نوع الاشتراك
            0,                  # عدد البوتات
            now,                # آخر نشاط
            "ar",               # اللغة
            "Direct" if not inviter_id else "Referral", # مصدر التسجيل
            ref_id,             # العمود 10: معرف إحالة (البصمة)
            0                   # العمود 11: رصيد
        ]
        
        # إضافة الصف في السطر الثاني (تحت الهيدر مباشرة)
...
def setup_bot_factory_database(bot_token=None):
    ...
    for config in structures:  
        try:  
            ...
            time.sleep(1.1) # فاصل زمني بعد عملية القراءة  
...
def add_new_coach_advanced(bot_token, coach_id, name, specialty, phone, branch_id, **kwargs):
    """إضافة مدرب بنظام الذاكرة المركزية والاعتماد على ID التيليجرام كمعرف وحيد"""
    try:
        from cache_manager import FACTORY_GLOBAL_CACHE, save_cache_to_disk
        
        # 1. المزامنة والربط الحي بالورقة
        if 'ss' not in globals() or ss is None: connect_to_google()
        ...
        # 2. [الخطوة الأولى]: الحقن الفوري في الذاكرة المركزية (RAM)
        new_coach_record = {
        ...
        # 3. [الخطوة الثانية]: بناء الصف الـ 15 عموداً بمطابقة تامة للمخطط
        # الترتيب: [bot_id, معرف_الفرع, ID, اسم_المدرب, التخصص, رقم_الهاتف, البريد_الإلكتروني, السيرة_الذاتية, رابط_الصورة, الحالة, bot_id_مكرر, معرف_الفرع_مكرر, اسم_الفرع, عدد_الدورات, تاريخ_التعاقد, ملاحظات, اسم_المستخدم]
        row = [
            str(bot_token),             # 1
            f"'{branch_id}",            # 2
        ...
        # استخدام الوسيط الآمن لمنع حظر API جوجل
        safe_api_call(current_sheet.append_row, row)
        
        # 4. [الخطوة الثالثة]: تحديث نظام المزامنة العالمي
        update_global_version(bot_token)
        
        print(f"✅ [نجاح]: المدرب {name} متاح الآن في الرام والشيت بالمعرف الرقمي: {coach_id}")
        return True
    ...
    try:
        ...
        for r in emp_records:
            if str(r.get("bot_id")) == str(bot_token):
                personnel.append({
                    "id": str(r.get("ID")),
                    "name": r.get("الاسم_الكامل") or r.get("اسم_المستخدم_تيلجرام") or "موظف بلا اسم",
                    "type": "موظف"
                })
...
def get_all_questions_from_bank(bot_token):
    """جلب كافة الأسئلة الحالية لهذا البوت من بنك الأسئلة"""
    try:
...
def add_new_employee_advanced(bot_token, employee_id, name, job_title, phone, branch_id, **kwargs):
    """إضافة كادر (مدرب/موظف) بـ 43 عموداً - أرقام نقية - تعبئة آلية للرتبة"""
    try:
        from cache_manager import FACTORY_GLOBAL_CACHE, save_cache_to_disk
        if 'ss' not in globals() or ss is None: connect_to_google()
        ...
        # 2. بناء الصف الـ 43 عموداً (الالتزام بالترتيب المتفق عليه)
        row = [""] * 43 
        row[0] = str(bot_token)            # 1. bot_id
        row[1] = int(branch_id)             # 2. معرف_الفرع (رقم نقي بدون ')
        ...
        row[10] = str(job_title)            # 11. التخصص (مجال الكادر)
        ...
        row[41] = str(role_tag)             # 42. الرتبة (تعبئة آلية: مدرب/موظف)
        row[42] = str(branch_name)          # 43. اسم_الفرع (تعبئة آلية)

        # 3. الحفظ كبيانات RAW لضمان عدم وجود علامة تنصيص
...
def get_active_gift_link(bot_token, user_id):
    ...
    try:
        ...
        for r in records:
            if (str(r.get("bot_id")) == str(bot_token) and 
                str(r.get("معرف_المسوق")) == str(user_id) and # نستخدم هذا العمود لتخزين ID المهدِي
                str(r.get("حالة_الكوبون")) == "نشط"):
                return r.get("معرف_الكوبون") # يعيد كود الهدية النشط
        return None
    except:
        return None
