...
def add_new_employee_advanced(bot_token, employee_id, name, job_title, phone, branch_id, **kwargs):
    """إضافة موظف جديد بـ 41 عموداً بمطابقة تامة للمخطط والاعتماد على الذاكرة المركزية"""
    try:
        from cache_manager import FACTORY_GLOBAL_CACHE, save_cache_to_disk
        if 'ss' not in globals() or ss is None: connect_to_google()
        ...
        # 1. تحديث الذاكرة المركزية RAM فوراً لضمان السرعة
        new_employee_record = {
            "bot_id": str(bot_token),
            "معرف_الفرع": branch_id,
            "ID": str(employee_id),
            ...
            "رقم_الهاتف": str(phone),
            "تاريخ_التعيين": today_date,
            "حالة_الحساب": "نشط",
            "اسم_المستخدم_تيلجرام": f"@{username}" if username != "بدون" else "بدون"
        }
        
        if "إدارة_الموظفين" not in FACTORY_GLOBAL_CACHE["data"]:
            FACTORY_GLOBAL_CACHE["data"]["إدارة_الموظفين"] = []
        FACTORY_GLOBAL_CACHE["data"]["إدارة_الموظفين"].append(new_employee_record)
        save_cache_to_disk()

        # 2. بناء الصف الـ 41 عموداً بمطابقة تامة لمخططك
        row = [None] * 41 # إنشاء صف فارغ وتعبئته بالترتيب
        row[0] = str(bot_token)           # bot_id
        row[1] = f"'{branch_id}"          # معرف_الفرع
        row[2] = f"'{employee_id}"        # ID (تيليجرام)
        row[3] = f"EMP-{str(employee_id)[:4]}" # معرف_الموظف (توليد داخلي)
        row[4] = str(name)                # الاسم_الكامل
        row[10] = "عام"                   # التخصص (افتراضي)
        row[11] = str(job_title)          # المسمى_الوظيفي
        row[17] = f"'{phone}"             # رقم_الهاتف
        row[20] = kwargs.get('email', "-") # البريد_الإلكتروني
        row[23] = today_date              # تاريخ_التعيين
        row[28] = "نشط"                   # الحالة_الوظيفية
        row[37] = "موظف"                  # الصلاحيات (مبدئية)
        row[39] = "نشط"                   # حالة_الحساب
        row[40] = f"@{username}" if username != "بدون" else "بدون" # اسم_المستخدم_تيلجرام (العمود الأخير)

        # تحويل كافة الـ None إلى نصوص فارغة لتجنب مشاكل جوجل شيت
        row = [str(x) if x is not None else "" for x in row]

        # 3. الكتابة الآمنة في جوجل شيت
        safe_api_call(current_sheet.append_row, row)
        
        # 4. تحديث نبضة المزامنة
        update_global_version(bot_token)
        
        print(f"✅ تم تسجيل الموظف {name} في الفرع {branch_name}")
        return True
    except Exception as e:
        logger.error(f"❌ خطأ إضافة موظف: {e}")
        return False
